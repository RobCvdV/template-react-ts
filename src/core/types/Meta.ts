...
import { isDefined, List, on, Optional, toList, use } from "@core";
...
class ClassMeta {
  ...
  entries = <T = unknown>(): List<Entry<T>> =>
    toList([
      ...Object.entries(this.subject),
      ...Object.entries(Object.getPrototypeOf(this.subject)),
    ]) as List<Entry<T>>;
  ...
  parse = <Out, Value>(
    ...
    return this.entries<Value>().reduce(
      (a, [k, val]) =>
        use(f(val), (v) =>
          isDefined(v) || !skipUndefined ? on(a, (a) => (a[k] = v)) : a,
        ),
      initial,
    );
  };
  ...
  properties = (key?: string): List<PropertyMeta> =>
    toList([
      ...Object.getOwnPropertyNames(this.subject),
      ...Object.getOwnPropertyNames(Object.getPrototypeOf(this.subject)),
    ])
  ...
  values = <T = unknown>(): List<T> =>
    toList([
      ...Object.values<T>(this.subject),
      ...Object.values<T>(Object.getPrototypeOf(this.subject)),
    ]);

  property = (property: string | symbol): PropertyMeta =>
    new PropertyMeta(this.subject, property);
}
...
class PropertyMeta {
  ...
  get = <T>(key: string): Optional<T> =>
    isDefined(this.data) && isDefined(this.data[key])
      ? (this.data[key] as T)
      : undefined;
...
export const meta = (subject: unknown): ClassMeta =>
  new ClassMeta(subject ?? {});

export const entries = <T = unknown>(
  subject: Record<string, T>,
): List<Entry<T>> => meta(subject).entries<T>();
