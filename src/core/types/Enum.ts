...
export abstract class Enum {
  ...
  protected static allTuple<E extends Enum>(): Record<Id, E> {
    return (
      meta(this).get(`all-${this.name}`) ??
      meta(this).set(
        `all-${this.name}`,
        meta(this).values<E>().filter(isEnum).toObject("id"),
      )
    );
  }

  static filter<E extends Enum>(
    p: (value: E, index: number, array: E[]) => unknown,
    params?: unknown,
  ): List<E> {
    return this.all<E>().filter(p, params);
  }

  static first<E extends Enum>(
    p?: (value: E, index: number, array: E[]) => unknown,
    params?: unknown,
  ): E {
...
export const isEnum: TypeGuard<Enum> = (e?: unknown): e is Enum =>
  isDefined(e) && e instanceof Enum && isAn<Enum>(e, "name", "id", "code");
