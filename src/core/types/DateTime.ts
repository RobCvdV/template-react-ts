...
export class DateTime extends Value<Optional<string>> {
  constructor(
    value?: string | number | Date | DateTime | null,
    format?: string,
  ) {
    super(
      choose(value)
        .type(isString, (v) =>
          format
            ? LuxonDateTime.fromFormat(v, format, { setZone: true })
            : LuxonDateTime.fromISO(v, { setZone: true }),
        )
  ...
  from(param?: string | DateTime, other?: string): string {
    ...
    const locale: string =
      (isString(param) ? param : undefined) ?? other ?? "en";
    return ifDefined(
      date,
      (d: any) => this.utc.setLocale(locale).toRelative({ base: d.utc }),
  ...
  add(n: number | Duration, unit?: DurationUnit): DateTime {
    return new DateTime(
      this.luxon
        .plus(isNumber(n) ? { [unit ?? ("day" as string)]: n } : n)
        .toISO(),
    );
  }
  ...
  subtract(n: number | Duration, unit?: DurationUnit): DateTime {
    return new DateTime(
      this.luxon
        .minus(isNumber(n) ? { [unit ?? ("day" as string)]: n } : n)
        .toISO(),
    );
  }
...
export const isDateTime = (dt?: unknown): dt is DateTime =>
  isDefined(dt) && dt instanceof DateTime;

export const dt = (dt?: unknown): DateTime =>
  isString(dt) || isNumber(dt) || dt instanceof Date
    ? new DateTime(dt)
    : new DateTime();
