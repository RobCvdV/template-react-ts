...
class CaseBuilder<V> {
  ...
  is = {
    defined: <T>(prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
      new Case<T, V>(this.v).case(isDefined(prop(this.v)), out),
    empty: <T>(prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
      new Case<T, V>(this.v).case(isEmpty(prop(this.v)), out),
    valid: <T>(prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
      new Case<T, V>(this.v).case((prop(this.v) as Validatable).isValid, out),
    not: {
      defined: <T>(prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
        new Case<T, V>(this.v).case(!isDefined(prop(this.v)), out),
      empty: <T>(prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
        new Case<T, V>(this.v).case(!isEmpty(prop(this.v)), out),
      valid: <T>(prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
        new Case<T, V>(this.v).case(
          !(prop(this.v) as Validatable).isValid,
          out,
        ),
    },
  };
...
class Case<T, V = unknown> {
  ...
  is = {
    defined: (prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
      new Case<T, V>(this.value).case(isDefined(prop(this.value)), out),
    empty: (prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
      new Case<T, V>(this.value).case(isEmpty(prop(this.value)), out),
    valid: (prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
      new Case<T, V>(this.value).case(
        (prop(this.value) as Validatable).isValid,
        out,
      ),
    not: {
      defined: (prop: Func<unknown, V>, out: Get<T, V>): Case<T, V> =>
