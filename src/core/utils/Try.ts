...
export abstract class Try<T = unknown> implements Validatable {
  is = {
    defined: (prop?: Func<unknown, T>): Try<T> =>
      this.filter((v) => isDefined(prop ? prop(v) : v)),
    empty: (prop?: Func<unknown, T>): Try<T> =>
      this.filter((v) => isEmpty(prop ? prop(v) : v)),
    valid: (prop?: Func<unknown, T>): Try<T> =>
      this.filter((v) => ((prop ? prop(v) : v) as Validatable).isValid),
    true: (prop?: Func<unknown, T>): Try<T> =>
      this.filter((v) => isTrue(prop ? prop(v) : v)),
  ...
  abstract recoverFrom(
    type: Constructor<Error>,
    f: Func<T | Try<T>, Error>,
  ): Try<T>;
...
export const tryTo = <T>(c: Get<T | Try<T>>, ...args: unknown[]) =>
  Try.of<T>(c, ...args);
